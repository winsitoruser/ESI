...
description: Generate production-grade UI using real component patterns and best practices from 60+ documented interface components. Use when building or redesigning Humanify HRIS (/humanify, /platform, ESS/MSS), SIMESI HQ (/hq), dashboards, forms, tables, navigation, or any web UI — ensures modern SaaS-quality output grounded in design-system conventions and this repo's tokens.
...
**Before writing any UI code**, consult this skill to select the right components and follow their best practices. Read [components.md](components.md) for the full reference. For this monorepo, also read [project-humanify.md](project-humanify.md) and reuse existing Humanify/SIMESI chrome and CSS tokens instead of inventing a new look.
...
### Step 0 — Project context (this repo)

If the change touches Humanify or SIMESI UI, read [project-humanify.md](project-humanify.md) first:
- Pick the correct product surface and preset
- Reuse `styles/humanify-tokens.css` and existing layout components
- Do not mix Humanify brand chrome into `/hq` (SIMESI)
