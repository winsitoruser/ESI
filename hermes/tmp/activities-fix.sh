...
import { Phone, Mail, Calendar, MessageCircle, CheckCircle, Trash2 } from 'lucide-react';
...
export default function ActivitiesTab({ activities, followUps, onAdd, onCompleteFollowUp, onDelete, canDelete, fmtDate, t }: ActivitiesTabProps) {
  return (
    <>
      ...
      {activities.length > 0 && (
        <CommChart activities={activities} followUps={followUps} t={t} onCompleteFollowUp={onCompleteFollowUp} fmtDate={fmtDate} />
      )}
