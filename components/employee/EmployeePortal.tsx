...
const HomeTab = dynamic(() => import('@/components/employee/tabs/HomeTab'), { loading: () => <TabSkeleton />, ssr: false });
...
export type EmployeePortalTab = TabKey;

const VALID_TABS = new Set<string>([
  'home', 'attendance', 'overtime', 'kpi', 'leave', 'claims', 'travel', 'visit', 'mf',
  'profile', 'files', 'manager', 'payslip', 'disciplinary', 'surveys', 'training',
]);
...
export default function EmployeePortal({ initialTab }: { initialTab?: TabKey } = {}) {
  ...
  useEffect(() => {
    ...
    const tab = (router.query.tab as string) || initialTab;
    if (tab && VALID_TABS.has(tab)) {
      setActiveTab(tab as TabKey);
    }
  }, [mounted, router.query.tab, initialTab]);
  ...
  const getGreeting = () => {
    ...
    return { text: 'Selamat Malam', icon: <Moon className="w-5 h-5 text-slate-400" /> };
  };
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      {ocrScanning && <span className="text-xs text-teal-600 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Membaca struk...</span>}
  ...
  const renderProfile = () => (
    <div className="space-y-4">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-teal-900 p-6 text-white text-center shadow-xl">
        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1 ${
              visitViewMode === 'team' ? 'bg-white text-teal-700 shadow-sm' : 'text-slate-500'
            }`}
          >
      ...
      {visitViewMode === 'team' && isManagerPortal ? (
        <>
          ...
          <div className="flex gap-2">
            ...
            />
            <button type="button" onClick={fetchTeamVisits} className="p-2.5 rounded-xl bg-teal-50 text-teal-700 active:scale-95">
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-teal-600 font-medium">{tv.employee_name}</p>
  ...
  const moreMenuItems: { key: TabKey; icon: any; label: string; desc: string; color: string }[] = [
    ...
    { key: 'surveys', icon: ClipboardList, label: 'Survei', desc: 'Survei engagement & feedback HR', color: 'bg-teal-100 text-teal-600' },
    { key: 'training', icon: GraduationCap, label: 'Training & LMS', desc: 'Pelatihan, ujian & kompetensi', color: 'bg-slate-100 text-slate-700' },
    ...(isMfAgent ? [{ key: 'mf' as TabKey, icon: Building2, label: 'Pembiayaan Lapangan', desc: 'Koleksi, portofolio & komisi', color: 'bg-slate-100 text-slate-700' }] : []),
    ...
    { key: 'travel',   icon: Plane,      label: 'Perjalanan',  desc: 'Perjalanan dinas',            color: 'bg-cyan-100 text-cyan-700' },
          ...
          <>
            ...
            <div className="absolute top-[3.25rem] right-3 left-3 z-50 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-200/60 max-h-80 overflow-y-auto">
              <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                ...
                {unreadCount > 0 && <span className="text-[10px] font-semibold text-teal-700 bg-teal-50 px-2 py-0.5 rounded-full ring-1 ring-teal-200">{unreadCount} baru</span>}
              </div>
          ...
          <div className="mx-3 mb-2 rounded-2xl bg-white/90 backdrop-blur-xl border border-slate-200/70 shadow-[0_-4px_24px_rgba(15,23,42,0.08)]">
            ...
            {tabs.map(tab => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <span className="absolute inset-x-2 top-1 bottom-1 rounded-xl bg-teal-50 -z-10" />
                  )}
                  <div className={`relative p-1 ${isActive ? 'text-teal-600' : ''}`}>
                    <tab.icon className={`w-5 h-5 ${isActive ? 'text-teal-600' : 'text-slate-400'}`} />
                  </div>
                  <span className={`text-[9px] font-semibold leading-none tracking-wide ${isActive ? 'text-teal-700' : 'text-slate-400'}`}>
                    {tab.label}
                  </span>
                  {tab.badge != null && tab.badge > 0 && (
                    <span className="absolute top-0.5 right-2 min-w-[14px] h-3.5 px-0.5 bg-teal-600 rounded-full text-[8px] text-white font-bold flex items-center justify-center ring-2 ring-white">
