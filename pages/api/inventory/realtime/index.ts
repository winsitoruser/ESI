...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('Inventory Realtime API Error: (table may not exist):', (error as any)?.message || error);
...
async function getRealtimeData(
  ...
  try {
    ...
    console.warn('Error fetching realtime inventory: (table may not exist):', (error as any)?.message || error);
    return res.status(200).json({
      success: true,
