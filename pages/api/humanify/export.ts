...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!['admin', 'hq_admin', 'hq_manager', 'owner', 'hr_manager', 'super_admin'].includes(userRole || '')) {
    ...
    console.warn('HRIS Export API Error: (table may not exist):', (error as any)?.message || error);
...
async function exportEmployees(
  ...
  try {
    if (!sequelize) throw new Error('sequelize unavailable');
...
async function exportAttendance(
  ...
  try {
    ...
    const attendance = await sequelize.query(`
      SELECT 
        COALESCE(e.employee_code, e.id::text) as "ID Karyawan",
        e.name as "Nama",
        e.position as "Jabatan",
        b.name as "Cabang",
        COALESCE(SUM(CASE WHEN a.status IN ('present', 'work_from_home') THEN 1 ELSE 0 END), 0) as "Hadir",
        ...
        COALESCE(SUM(CASE WHEN a.status IN ('work_from_home', 'wfh') THEN 1 ELSE 0 END), 0) as "WFH",
        COUNT(a.id) as "Total Hari",
        ROUND(COALESCE(SUM(CASE WHEN a.status IN ('present', 'late', 'work_from_home', 'wfh') THEN 1 ELSE 0 END), 0) * 100.0 / NULLIF(COUNT(a.id), 0), 1) as "Tingkat Kehadiran (%)"
      FROM employees e
      LEFT JOIN branches b ON e.branch_id = b.id
      LEFT JOIN employee_attendance a ON a.employee_id::text = e.id::text AND TO_CHAR(a.date, 'YYYY-MM') = :period
      ${whereClause}
      GROUP BY e.id, e.employee_code, e.name, e.position, b.name
      ORDER BY e.name
    `, {
      replacements,
      type: QueryTypes.SELECT
