...
import {
  generateDisbursementFile, getMockDisbursementRows, type BankFormat,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    if (action === 'preview') {
      const rows = getMockDisbursementRows();
      const total = rows.reduce((s, r) => s + r.amount, 0);
      return res.json({ success: true, data: { rows, total, count: rows.length } });
    }
  ...
  if (req.method === 'POST') {
    ...
    const bankFormat = (format as BankFormat) || 'generic';
    const data = rows?.length ? rows : getMockDisbursementRows();
