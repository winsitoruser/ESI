...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (method === 'GET') {
      ...
      if (action === 'outsourcing-pipeline' || action === 'pipeline') {
        const [pipeline] = await sequelize.query(`
          SELECT 
            (SELECT COUNT(*)::int FROM hris_candidates WHERE tenant_id = :tid AND COALESCE(current_stage, status) NOT IN ('rejected','withdrawn')) as recruiting,
    ...
    console.warn('[training-development API]', error.message);
