...
import { HRIS_DEPARTMENT_CODES } from '../../../lib/hris/master-data';

const DEPARTMENTS = HRIS_DEPARTMENT_CODES;
const PAY_TYPES = ['monthly','daily','hourly','weekly','project','piecework','commission','base_plus_commission'];
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('Payroll Bulk API Error: (table may not exist):', (error as any)?.message || error);
...
async function downloadTemplateExcel(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('Excel template error: (table may not exist):', (e as any)?.message || e);
...
async function getEmployeeList(req: NextApiRequest, res: NextApiResponse, session: any) {
  ...
  try {
    const [rows] = await sequelize.query(`
      ...
      FROM employees WHERE LOWER(COALESCE(status, 'active')) = 'active' OR is_active = true ORDER BY name LIMIT 500
    `);
...
async function handleBulkSave(req: NextApiRequest, res: NextApiResponse, session: any) {
  try {
    ...
    if (sequelize) {
      try {
        const [emps] = await sequelize.query(`SELECT id, employee_id as emp_code, name FROM employees WHERE LOWER(COALESCE(status, 'active')) = 'active' OR is_active = true`);
    ...
    console.warn('Bulk save error: (table may not exist):', (e as any)?.message || e);
    return res.status(500).json({ success: false, error: e.message });
  }
}
