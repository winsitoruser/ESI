  ...
  try {
    if (sequelize) {
      ...
      try {
        ...
        const [rows] = await sequelize.query(`
          SELECT e.id, e.employee_code AS employee_id, e.name, e.position, e.department,
                 e.branch_id, NULL::varchar AS work_location,
                 b.name as branch_name
          FROM employees e
          LEFT JOIN branches b ON e.branch_id = b.id
          WHERE (e.is_active = true OR e.status = 'active' OR e.status = 'ACTIVE' OR e.status IS NULL)
...
async function calculateBranchKPIs(period: string, filterBranchId?: string) {
  ...
  try {
    ...
    // Backfill branch_id on employee_kpis from employees when missing
    try {
      await sequelize.query(`
        UPDATE employee_kpis ek SET branch_id = e.branch_id
        FROM employees e WHERE ek.employee_id = e.id AND ek.branch_id IS NULL AND e.branch_id IS NOT NULL
      `);
    } catch { /* ignore */ }
    ...
    for (const branch of branches) {
      ...
      try {
        const [opsData] = await sequelize.query(`
          ...
          FROM employee_kpis ek
          LEFT JOIN employees e ON ek.employee_id = e.id
          WHERE COALESCE(ek.branch_id, e.branch_id) = :branchId AND ek.period = :period
