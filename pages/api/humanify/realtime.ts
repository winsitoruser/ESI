...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('HRIS Realtime API Error: (table may not exist):', (error as any)?.message || error);
  ...
  try {
    ...
    const employees = await sequelize.query(`
      ...
      LEFT JOIN (
        SELECT 
          employee_id,
          SUM(CASE WHEN status IN ('present', 'work_from_home') THEN 1 ELSE 0 END) as present_days,
      ...
      ) att ON att.employee_id::text = e.id::text
      ${whereClause}
      ORDER BY e.name
    `, {
      replacements,
