...
async function q(sql: string, replacements?: any): Promise<any[]> {
  ...
  try {
    ...
    console.warn('Audit Query Error:', e.message);
    return [];
  }
}
...
async function qExec(sql: string, replacements?: any): Promise<boolean> {
  ...
  try {
    ...
    console.warn('Audit Exec Error:', e.message);
    return false;
  }
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('Audit Trail Error: (table may not exist):', (error as any)?.message || error);
