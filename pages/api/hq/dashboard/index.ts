...
/**
 * Build tenant filter condition and params based on user's tenantId
 * - If tenantId exists: adds filter condition and includes tenantId in params
 * - If tenantId is null/undefined (super_admin): no tenant filter (sees all tenants)
 * 
 * @param tenantId - User's tenant ID from session
 * @param tableAlias - Table alias to use (e.g., 'b' for branches, 'pt' for pos_transactions)
 * @returns { filter: string, params: any }
 */
function buildTenantContext(
  tenantId: string | null | undefined,
  tableAlias: string = 'b'
): { filter: string; params: any; hasTenant: boolean } {
  if (tenantId) {
    return {
      filter: `AND ${tableAlias}.tenant_id = :_tenantId`,
      params: { _tenantId: tenantId },
      hasTenant: true
    };
  }
  // super_admin case - no tenant restriction
  return {
    filter: '',
    params: {},
    hasTenant: false
...
export default async function handler(
  ...
  try {
    ...
    // Get tenantId from session - may be null for super_admin
    const userTenantId = session.user.tenantId || null;
    const isSuperAdmin = session.user.role === 'super_admin' && !userTenantId;
    ...
    if (req.method === 'GET') {
      ...
      switch (view) {
        case 'overview':
          data = await getHQOverview(userTenantId, dateFilter, branchFilter, branchParams);
          break;

        case 'branches':
          data = await getBranchesStatus(userTenantId, dateFilter, branchFilter, branchParams);
          break;

        case 'departments':
          data = await getDepartmentsStatus(userTenantId, dateFilter, branchFilter, branchParams);
          break;

        case 'alerts':
          data = await getHQAlerts(userTenantId, branchFilter, branchParams);
          break;

        case 'trends':
          data = await getHQTrends(userTenantId, timeRange, branchFilter, branchParams);
          break;
