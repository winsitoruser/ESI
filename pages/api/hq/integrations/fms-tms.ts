...
const q = async (sql: string, replacements?: any) => {
  ...
  catch (e: any) { console.warn('FMS-TMS Q: (table may not exist):', e.message); return []; }
};
...
const qExec = async (sql: string, replacements?: any) => {
  ...
  catch (e: any) { console.warn('FMS-TMS Exec: (table may not exist):', e.message); return false; }
};
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('FMS-TMS Integration error: (table may not exist):', (error as any)?.message || error);
