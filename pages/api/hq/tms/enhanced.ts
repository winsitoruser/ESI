...
const q = async (sql: string, replacements?: any) => {
  ...
  catch (e: any) { console.warn('TMS-E Q: (table may not exist):', e.message); return []; }
};
...
const qExec = async (sql: string, replacements?: any) => {
  ...
  catch (e: any) { console.warn('TMS-E Exec: (table may not exist):', e.message); return false; }
};
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('TMS Enhanced API error: (table may not exist):', (error as any)?.message || error);
