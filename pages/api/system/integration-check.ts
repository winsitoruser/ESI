...
async function checkModules(sequelize: any): Promise<IntegrationStatus[]> {
  ...
  modules.push({
    module: 'Conservation (ESI)',
    status: 'ok',
    details: 'Modul konservasi ESI: proyek, aset, basis pengetahuan.',
    apiEndpoints: ['/api/hq/project-management', '/api/hq/assets', '/api/hq/knowledge-base'],
  });
...
async function checkHQModule(sequelize: any): Promise<IntegrationStatus> {
  ...
  try {
    ...
    if (missingTables.length === 0) {
      return {
        module: 'HQ (ESI single-org)',
        status: 'ok',
        details: 'Organisasi tunggal ESI — API cabang menggunakan stub HQ.',
        tables: existingTables,
        apiEndpoints,
      };
    }
    ...
    return {
      module: 'HQ (ESI single-org)',
      status: 'warning',
      details: `Missing tables: ${missingTables.join(', ')}. Stub API tetap tersedia.`,
      tables: existingTables,
      apiEndpoints,
    };
  } catch (error: any) {
    return {
      module: 'HQ (ESI single-org)',
      status: 'warning',
      details: `Database check failed. Stub APIs available.`,
      apiEndpoints,
    };
  }
}
