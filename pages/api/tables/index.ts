...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('Tables API error: (table may not exist):', (error as any)?.message || error);
...
async function getTables(req: NextApiRequest, res: NextApiResponse, Table: any) {
  try {
    ...
    console.warn('getTables error: (table may not exist):', (error as any)?.message || error);
    return res.status(200).json({ success: true, data: [] });
  }
}
