...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.warn('Reservations API error: (table may not exist):', (error as any)?.message || error);
...
async function getReservations(req: NextApiRequest, res: NextApiResponse, Reservation: any) {
  try {
    ...
    console.warn('getReservations error: (table may not exist):', (error as any)?.message || error);
    return res.status(200).json({ success: true, data: [] });
  }
}
