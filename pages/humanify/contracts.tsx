...
export default function ContractsPage() {
  ...
  async function handleSave() {
    ...
    try {
      const url = editing
        ? `/api/humanify/lifecycle?action=contract&id=${editing.id}`
        : '/api/humanify/lifecycle?action=contract';
  ...
  async function handleRenew() {
    if (!selected) return;
    try {
      const res = await fetch(`/api/humanify/lifecycle?action=contract-renew&id=${selected.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
      });
  ...
  async function handleTerminate(c: Contract) {
    ...
    if (!reason) return;
    try {
      const res = await fetch(`/api/humanify/lifecycle?action=contract-terminate&id=${c.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete(c: Contract) {
    ...
    try {
      await fetch(`/api/humanify/lifecycle?action=contract&id=${c.id}`, { method: 'DELETE' });
            ...
            <div className="p-5 space-y-3">
              {!editing && (
                <EmployeePicker
                  ...
                  onChange={(emp: PickedEmployee | null) => {
                    if (!emp) {
                      setForm(f => ({ ...f, employeeId: undefined, employeeName: '', position: '', department: '' }));
                      return;
                    }
                    setForm(f => ({
                      ...f,
              ...
              {editing ? (
                <div className="grid grid-cols-2 gap-3">
                  <ReadOnlyField label="UID" value={(form as any).employeeUid || String(form.employeeId || '-')} />
                  <ReadOnlyField label="Nama" value={form.employeeName || '-'} />
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  <ReadOnlyField label="UID" value={(form as any).employeeUid || '-'} />
                  <ReadOnlyField label="Nama" value={form.employeeName || '-'} />
                </div>
              )}
              <div className="grid grid-cols-2 gap-3">
