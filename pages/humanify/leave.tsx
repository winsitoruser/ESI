...
const MOCK_LEAVE_REQUESTS: LeaveRequest[] = [
  ...
  { id: 'lr4', employee_name: 'Rina Anggraini', position: 'Head Chef', department: 'PRODUCTION', leave_type: 'annual', start_date: '2026-03-10', end_date: '2026-03-14', total_days: 5, reason: 'Pulang kampung', status: 'approved', approved_at: '2026-03-08' },
...
const MOCK_LEAVE_BALANCES = [
  ...
  { employee_name: 'Rina Anggraini', department: 'PRODUCTION', annual: { total: 12, used: 8, remaining: 4 }, sick: { total: 14, used: 0, remaining: 14 } },
];
...
export default function LeaveManagementPage() {
  ...
  const handleApproveStep = async (requestId: string) => {
    try {
      const res = await fetch('/api/humanify/leave-management?action=approve', {
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      const res = await fetch('/api/humanify/leave-management?action=reject', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/humanify/leave-management?action=approval-config`, {
  ...
  const handleDeleteConfig = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/humanify/leave-management?action=approval-config&id=${id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  const handleDeleteType = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/humanify/leave-management?action=type&id=${id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  const handleInitBalances = async () => {
    try {
      const res = await fetch('/api/humanify/leave-management?action=balance-init', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="col-span-3">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Posisi/Jabatan (pisahkan koma)</label>
                    <input type="text"
                      value={(typeForm.applicable_positions || []).join(', ')}
                      onChange={e => setTypeForm(f => ({
                        ...f,
                        applicable_positions: e.target.value.split(',').map(s => s.trim()).filter(Boolean),
                      }))}
                      className="w-full px-3 py-2 border rounded-lg text-sm"
                      placeholder="Contoh: Branch Manager, Sales Supervisor" />
                    <p className="text-[10px] text-gray-400 mt-0.5">Kosong = semua posisi.</p>
                  </div>
                </div>
              </div>
