...
export default function DeviceManagementPage() {
  ...
  const { branches } = useHrisMasterData();
  ...
  const fetchDevices = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/humanify/attendance/devices?${params}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const {
        id, branch, isOnline, registeredUsers, totalSynced,
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json.success) {
        toast.error(json.error || 'Gagal menyimpan device');
        return;
      }
      toast.success(isEdit ? 'Device berhasil diupdate' : 'Device berhasil ditambahkan');
      if (json._mock) toast('Mode demo — jalankan migrasi DB untuk menyimpan permanen', { icon: 'ℹ️' });
  ...
  const handleDelete = async (deviceId: string) => {
    ...
    try {
      const res = await fetch(`/api/humanify/attendance/devices?id=${deviceId}`, { method: 'DELETE' });
  ...
  const openAdd = () => {
    ...
    setFormData({
      ...
      syncInterval: 5, maxCapacity: 1000, location: '',
      branchId: branches[0]?.id || '', notes: ''
    });
