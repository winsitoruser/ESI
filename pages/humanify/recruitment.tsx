...
const MOCK_OPENINGS = [
  ...
  { id: 'op2', title: 'Senior Barista', department: 'PRODUCTION', location: 'Cabang Bali', type: 'full_time', priority: 'medium', salary_min: 5000000, salary_max: 7000000, status: 'open', applicants: 45, deadline: '2026-04-15', created_at: '2026-03-01' },
...
export default function RecruitmentPage() {
  ...
  const fetchIntegrations = useCallback(async () => {
    try {
      const res = await fetch('/api/humanify/integrations?action=recruitment');
      const data = await res.json();
  ...
  const fetchScreening = useCallback(async () => {
    try {
      const res = await fetch('/api/humanify/recruitment?action=screening');
      const data = await res.json();
      if (data.data) setScreeningResults(Array.isArray(data.data) ? data.data : []);
    } catch (e) { console.warn('Failed to fetch screening:', e); }
  }, []);

  useEffect(() => {
    setLoading(true);
    Promise.all([fetchOpenings(), fetchCandidates(), fetchAnalytics(), fetchIntegrations()]).finally(() => setLoading(false));
  }, [fetchOpenings, fetchCandidates, fetchAnalytics, fetchIntegrations]);
  ...
  async function handleDeleteOpening(id: string) {
    ...
    try {
      const res = await fetch(`/api/humanify/recruitment?action=delete-opening&id=${id}`, { method: 'DELETE' });
      const data = await res.json();
  ...
  async function handleAdvanceCandidate(candidate: any) {
    ...
    try {
      const res = await fetch('/api/humanify/recruitment?action=update-candidate', {
  ...
  const departments = masterDepts.map((d) => d.code);
  ...
  const tabs: { key: TabKey; label: string; icon: any }[] = [
    ...
    { key: 'screening', label: 'AI Screening', icon: Star },
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                        <span className="flex items-center gap-1"><Briefcase className="w-3 h-3" />{getDepartmentLabel(o.department)}</span>
              ...
              <form onSubmit={handleCreateOpening} className="p-5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-sm font-medium text-gray-700">Departemen *</label>
                    <DepartmentSelect value={createForm.department} onChange={(v) => setCreateForm({ ...createForm, department: v })} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm" required />
                  </div>
