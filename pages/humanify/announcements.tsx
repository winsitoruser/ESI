...
const CATEGORY_CONF: Record<string, { label: string; color: string }> = {
  general: { label: 'Umum', color: 'bg-[var(--hf-brand-100)] text-[color:var(--hf-brand)]' },
  ...
  training: { label: 'Training', color: 'bg-[var(--hf-brand-100)] text-[color:var(--hf-brand)]' },
};
...
export default function HRISAnnouncementsPage() {
  ...
  async function fetchAnnouncements() {
    setLoading(true);
    try {
      const res = await fetch('/api/humanify/engagement?action=announcements');
      ...
      if (Array.isArray(list)) {
        ...
        setItems(list.map((a: any) => ({
          ...
          publishDate: a.publish_date || a.publishDate || (a.published_at ? String(a.published_at).split('T')[0] : undefined),
          expireDate: a.expire_date || a.expireDate || (a.expires_at ? String(a.expires_at).split('T')[0] : undefined),
  ...
  async function handleSave() {
    ...
    try {
      const url = editing
        ? `/api/humanify/engagement?action=announcement&id=${editing.id}`
        : '/api/humanify/engagement?action=announcement';
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await fetch(`/api/humanify/engagement?action=announcement&id=${id}`, { method: 'DELETE' });
    } catch {}
  ...
  async function togglePin(item: Announcement) {
    ...
    try {
      await fetch(`/api/humanify/engagement?action=announcement&id=${item.id}`, {
    ...
    <HQLayout title="Pengumuman" subtitle="Broadcast pengumuman & kebijakan HR ke seluruh karyawan">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Aktif', value: items.filter(i => i.status === 'published').length, icon: Megaphone, color: 'text-[color:var(--hf-brand-600)]', bg: 'bg-[var(--hf-brand-100)]' },
            ...
            <div className="p-5 space-y-4">
              ...
              {form.targetAudience === 'department' && (
                <DepartmentSelect
                  ...
                  onChange={(v) => setForm((f) => ({ ...f, targetDepartment: v }))}
                  className="w-full px-3 py-2 border rounded-lg text-sm"
                />
              )}
