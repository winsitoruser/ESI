        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl border shadow-sm p-5">
              <h3 className="font-semibold flex items-center gap-2 mb-4"><Network className="w-4 h-4 text-[color:var(--hf-brand-600)]" /> Struktur & Job Architecture</h3>
              <div className="space-y-2">
                {data.structure.map((s: any) => (
                  <Link key={s.id} href={s.href} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 border group">
                    <div>
                      <p className="font-medium text-sm group-hover:text-[color:var(--hf-brand-600)]">{s.name}</p>
                      <p className="text-xs text-gray-400">{s.count}</p>
                    </div>
                    <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[color:var(--hf-brand-500)]" />
                  </Link>
                ))}
              </div>
            </div>
          ...
          <div className="bg-[var(--hf-brand-50)] border border-[var(--hf-brand-100)] rounded-xl p-4 text-sm">
            ...
            <Link href="/humanify/employees" className="text-[color:var(--hf-brand-600)] underline">Database Karyawan</Link>,{' '}
            <Link href="/humanify/certificates" className="text-[color:var(--hf-brand-600)] underline">Certificate Registry</Link>, dan{' '}
            <Link href="/humanify/contracts" className="text-[color:var(--hf-brand-600)] underline">Kontrak & Reminder</Link>.
          </div>
        </div>
      </HQLayout>
    </PageGuard>
