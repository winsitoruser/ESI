...
> ℹ️ **`phase15-password-reset` di prod**: endpoint dengan sengaja **tidak** mengembalikan reset token (aman) karena SMTP belum dikonfigurasi & flag debug `HUMANIFY_PASSWORD_RESET_RETURN_TOKEN` off. Smoke butuh token untuk lanjut → "gagal" hanya keterbatasan harness terhadap prod yang di-hardening, **bukan** regresi. Untuk verifikasi flow reset: konfigurasikan `SMTP_*` (item follow-up) atau jalankan smoke di non-prod.
...
Kandidat lanjutan: Sentry aktif (set `SENTRY_DSN` + `npm i @sentry/node`) + uptime monitor (UptimeRobot/BetterStack) → `/api/health?deep=1`; MFA recovery codes + enforce 2FA per-kebijakan tenant; QR code lokal utk enrol MFA (hindari kirim secret ke pihak ketiga); wiring SAML ACS→session (samlify, maintenance window); notifikasi in-app real-time (WebSocket — polling 60s sudah live P21); proration downgrade; hard-delete offboarding otomatis via cron; rate-limit & login-guard store → Redis bila scale multi-instance. **Follow-up P23**: manajemen anggota (nonaktifkan/hapus user, ubah role dari halaman Tim), assign `role_id` RBAC lanjutan saat/ setelah accept (kini hanya legacy `role`), audit trail undangan. **Utang teknis (diselesaikan P24)**: ~~`createEmployee` single generate `employee_code` `EMP001` per-tenant~~ → sudah `EMP-<tenant6>-<seq>`; ~~IDOR update/hapus lintas-tenant~~ → sudah di-scope `tenantId`; ~~seat leak karyawan nonaktif~~ → metering by `status` + `is_active` disinkronkan. **Utang teknis tersisa**: (a) **SMTP prod belum dikonfigurasi** → verifikasi email (P7), reset password (P15), undangan (P23), digest (P12) tak terkirim; set `SMTP_*` di `.env` VPS. (b) Undangan hanya set legacy `role`, belum `role_id` RBAC. (c) Public API v1 minim (read `/api/v1/employees` saja). (d) SAML ACS→session belum tersambung. (e) rate-limit & login-guard in-memory (perlu Redis bila multi-instance).
