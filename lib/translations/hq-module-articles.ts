...
const art_id = {
  ...
  faq: {
    fnb: {
      q1: { q: 'Apakah SIMESI cocok untuk restoran dengan banyak cabang?', a: 'Ya, SIMESI dirancang untuk multi-branch operation. Anda bisa mengelola semua cabang dari satu dashboard HQ, dengan data yang tersinkronisasi real-time. Setiap cabang bisa memiliki konfigurasi menu, harga, dan pajak yang berbeda.' },
      ...
      q4: { q: 'Metode pembayaran apa saja yang didukung?', a: 'SIMESI POS mendukung tunai, QRIS (semua e-wallet), GoPay, OVO, Dana, ShopeePay, kartu debit/kredit (via payment gateway), dan split payment kombinasi. Anda juga bisa menambahkan metode kustom.' },
    ...
    retail: {
      q1: { q: 'Bisakah SIMESI terhubung ke Tokopedia dan Shopee?', a: 'Ya, melalui modul Marketplace Integration. Produk dan stok di-sync otomatis ke Tokopedia, Shopee, dan marketplace lain. Pesanan dari marketplace juga otomatis masuk ke sistem untuk diproses.' },
      ...
      q4: { q: 'Bagaimana menangani return dan refund?', a: 'POS SIMESI mendukung proses return dan refund langsung. Return bisa berupa tukar barang atau refund uang, dan stok otomatis di-update ke inventory. Semua transaksi return tercatat di audit log.' },
    ...
    manufacturing: {
      q1: { q: 'Bagaimana SIMESI membantu proses manufaktur?', a: 'SIMESI menyediakan modul Inventory untuk raw material management, recipe/BOM tracking, purchase order ke supplier, dan stock opname. Terintegrasi dengan Finance untuk costing dan HR untuk manajemen shift produksi.' },
    ...
    services: {
      q1: { q: 'Bagaimana SIMESI cocok untuk bisnis jasa profesional?', a: 'SIMESI menyediakan CRM untuk client management, Project Management untuk tracking proyek dan timesheet, Finance untuk invoicing dan billing, serta HRIS untuk kelola tim profesional Anda.' },
    ...
    logistics: {
      q1: { q: 'Apakah SIMESI cocok untuk perusahaan logistik?', a: 'Sangat cocok. Dengan modul TMS (Transportation Management) dan FMS (Fleet Management), Anda bisa mengelola seluruh operasional logistik: shipment, trip planning, carrier management, armada kendaraan, dan proof of delivery.' },
...
const art_en = {
  ...
  faq: {
    fnb: {
      q1: { q: 'Is SIMESI suitable for restaurants with multiple branches?', a: 'Yes, SIMESI is designed for multi-branch operations. You can manage all branches from one HQ dashboard with real-time synchronized data. Each branch can have different menu, pricing, and tax configurations.' },
      ...
      q4: { q: 'What payment methods are supported?', a: 'SIMESI POS supports cash, QRIS (all e-wallets), GoPay, OVO, Dana, ShopeePay, debit/credit cards (via payment gateway), and split payment combinations. You can also add custom methods.' },
    ...
    retail: {
      q1: { q: 'Can SIMESI connect to Tokopedia and Shopee?', a: 'Yes, through the Marketplace Integration module. Products and stock are auto-synced to Tokopedia, Shopee, and other marketplaces. Orders from marketplaces also automatically enter the system for processing.' },
      ...
      q4: { q: 'How to handle returns and refunds?', a: 'SIMESI POS supports direct return and refund processing. Returns can be exchanges or money refunds, and stock is auto-updated in inventory. All return transactions are recorded in the audit log.' },
    ...
    manufacturing: {
      q1: { q: 'How does SIMESI help manufacturing processes?', a: 'SIMESI provides Inventory module for raw material management, recipe/BOM tracking, purchase orders to suppliers, and stock opname. Integrated with Finance for costing and HR for production shift management.' },
    ...
    services: {
      q1: { q: 'How is SIMESI suitable for professional services?', a: 'SIMESI provides CRM for client management, Project Management for project and timesheet tracking, Finance for invoicing and billing, and HRIS to manage your professional team.' },
    ...
    logistics: {
      q1: { q: 'Is SIMESI suitable for logistics companies?', a: 'Absolutely. With TMS (Transportation Management) and FMS (Fleet Management) modules, you can manage all logistics operations: shipments, trip planning, carrier management, vehicle fleets, and proof of delivery.' },
...
const art_ja = {
  ...
  faq: {
    fnb: {
      q1: { q: 'SIMESIは複数店舗のレストランに適していますか？', a: 'はい、SIMESIはマルチブランチ運営向けに設計されています。1つのHQダッシュボードからすべての店舗をリアルタイムで管理でき、各店舗は独自のメニュー、価格、税金設定が可能です。' },
      ...
      q4: { q: 'どのような決済方法に対応していますか？', a: 'SIMESI POSは現金、QRIS（全電子ウォレット）、GoPay、OVO、Dana、ShopeePay、デビット/クレジットカード（決済ゲートウェイ経由）、分割払いに対応しています。カスタム方法の追加も可能です。' },
      q5: { q: '新しいレストランのセットアップにはどのくらいかかりますか？', a: '基本セットアップ（商品、カテゴリ、税金、レシート）は1-2日で完了できます。在庫、従業員、財務を含むフルセットアップは当社チームのサポートで3-5営業日かかります。' },
    },
    retail: {
      q1: { q: 'SIMESIはTokopediaやShopeeと接続できますか？', a: 'はい、マーケットプレイス連携モジュールで可能です。商品と在庫はTokopedia、Shopee、その他のマーケットプレイスに自動同期されます。マーケットプレイスからの注文も自動的にシステムに入ります。' },
      ...
      q4: { q: '返品と返金はどう処理しますか？', a: 'SIMESI POSは返品と返金の直接処理をサポートしています。商品交換または返金が可能で、在庫は自動更新されます。すべての返品取引は監査ログに記録されます。' },
    ...
    manufacturing: {
      q1: { q: 'SIMESIは製造プロセスをどのように支援しますか？', a: 'SIMESIは原材料管理、レシピ/BOM追跡、仕入先への発注、棚卸しのための在庫モジュールを提供します。原価計算のための財務とシフト管理のためのHRと統合されています。' },
    ...
    services: {
      q1: { q: 'SIMESIはプロフェッショナルサービスに適していますか？', a: 'SIMESIはクライアント管理のためのCRM、プロジェクトとタイムシート追跡のためのプロジェクト管理、請求のための財務、チーム管理のためのHRISを提供します。' },
    ...
    logistics: {
      q1: { q: 'SIMESIは物流会社に適していますか？', a: 'TMS（輸送管理）とFMS（フリート管理）モジュールで、出荷、トリップ計画、キャリア管理、車両フリート、配達証明などすべての物流業務を管理できます。' },
...
const art_zh = {
  ...
  faq: {
    fnb: {
      q1: { q: 'SIMESI适合多分店餐厅吗？', a: '是的，SIMESI专为多分店运营设计。您可以从一个HQ仪表板管理所有分店，数据实时同步。每个分店可以拥有不同的菜单、价格和税务配置。' },
      ...
      q4: { q: '支持哪些付款方式？', a: 'SIMESI POS支持现金、QRIS（所有电子钱包）、GoPay、OVO、Dana、ShopeePay、借记卡/信用卡（通过支付网关）和混合分期付款。您还可以添加自定义方式。' },
      q5: { q: '新餐厅的设置需要多长时间？', a: '基本设置（产品、类别、税务、收据）可在1-2天内完成。包括库存、员工和财务的完整设置在我们团队的指导下需要3-5个工作日。' },
    },
    retail: {
      q1: { q: 'SIMESI能连接Tokopedia和Shopee吗？', a: '是的，通过电商平台集成模块。产品和库存自动同步到Tokopedia、Shopee和其他平台。平台订单也自动进入系统进行处理。' },
      ...
      q4: { q: '如何处理退货和退款？', a: 'SIMESI POS支持直接退货和退款处理。退货可以是换货或退款，库存自动更新。所有退货交易记录在审计日志中。' },
    ...
    manufacturing: {
      q1: { q: 'SIMESI如何帮助制造流程？', a: 'SIMESI提供原材料管理、配方/BOM跟踪、供应商采购和盘点的库存模块。与财务集成进行成本核算，与HR集成进行生产班次管理。' },
    ...
    services: {
      q1: { q: 'SIMESI适合专业服务公司吗？', a: 'SIMESI提供客户管理的CRM、项目和工时跟踪的项目管理、发票和计费的财务，以及团队管理的HRIS。' },
    ...
    logistics: {
      q1: { q: 'SIMESI适合物流公司吗？', a: '非常适合。通过TMS和FMS模块，您可以管理所有物流业务：货运、行程规划、承运商管理、车队和交货证明。' },
